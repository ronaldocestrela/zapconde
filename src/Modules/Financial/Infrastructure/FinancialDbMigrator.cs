...
public static class FinancialDbMigrator
{
        ...
        CancellationToken ct = default)
    {
        ...
        var logger = scope.ServiceProvider.GetRequiredService<ILoggerFactory>()
            .CreateLogger("Modules.Financial.Infrastructure.FinancialDbMigrator");

        if (!dbContext.Database.IsRelational())
        {
            await dbContext.Database.EnsureCreatedAsync(ct);
            logger.LogInformation("Financial database ensured (in-memory provider).");
            return;
        }
        ...
        try
        {
            ...
            if (pendingMigrations.Count == 0)
            {
                logger.LogInformation(
                    "Financial database is already up to date ({DatabaseTarget}).",
                    databaseTarget);
                return;
            }

            logger.LogInformation(
                "Applying {MigrationCount} pending Financial migration(s) to {DatabaseTarget}: {Migrations}",
            ...
            logger.LogInformation(
                "Applied {MigrationCount} Financial migration(s) to {DatabaseTarget}.",
                pendingMigrations.Count,
                databaseTarget);
        }
        catch (Exception ex)
